...
class GoogleSheetsMotick:
    ...
    def leer_datos_scraper_reciente(self):
        ...
        CORREGIDO: Busca hojas SCR DD/MM/YY en lugar de Datos_
        """
        try:
            spreadsheet = self.client.open_by_key(self.sheet_id)
            ...
            # CORRECCION: Buscar hojas SCR en lugar de Datos_
            hojas_datos = []
                ...
                if hoja.startswith('SCR ') and len(hoja) == 10:  # "SCR 03/09/25" = 10 caracteres
                    try:
                        ...
                        if len(partes) == 3:
                            ...
                            ano_completo = f"20{ano_2d}" if int(ano_2d) <= 30 else f"19{ano_2d}"
                            fecha_str = f"{dia}/{mes}/{ano_completo}"
                            fecha_obj = datetime.strptime(fecha_str, "%d/%m/%Y")
                            hojas_datos.append((hoja, fecha_obj, fecha_str))
                            print(f"DEBUG: Hoja SCR detectada: {hoja} -> {fecha_str}")
                    except:
                        continue
    ...
    def guardar_historico_con_hojas_originales(self, df_historico, fecha_procesamiento):
        ...
        Mantiene la misma lÃ³gica que el script local pero en Google Sheets
        """
        try:
            spreadsheet = self.client.open_by_key(self.sheet_id)
